...
function extractAcceptanceCriteria(taskText: string): string[] {
	...
	const body = section ? section[1] : taskText;
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		const sorted = [...fileHits.entries()].sort((a, b) => b[1].size - a[1].size).slice(0, 25);
		...
		for (const p of literalPaths.slice(0, 3)) styleFiles.add(p);
		for (const [f] of sorted.slice(0, 3)) styleFiles.add(f);
		const styleEntries: string[] = [];
		for (const file of [...styleFiles].slice(0, 3)) {
			const style = detectFileStyle(cwd, file);
			if (style) styleEntries.push(`- ${file}: ${style}`);
		}
		if (styleEntries.length > 0) {
			sections.push(`\nDETECTED STYLES (your edits MUST match each file's style character-for-character):`);
			for (const e of styleEntries) sections.push(e);
		}

		const criteriaCount = countAcceptanceCriteria(taskText);
		const criteriaList = extractAcceptanceCriteria(taskText);
		if (criteriaCount > 0) {
			sections.push(`\nThis task has ${criteriaCount} acceptance criteria.`);
			if (criteriaList.length > 0) {
				sections.push("ACCEPTANCE CHECKLIST (implement every item — missing any criterion scores worse than extra lines):");
				for (let i = 0; i < criteriaList.length; i++) {
					sections.push(`  ${i + 1}. ${criteriaList[i]}`);
				}
			}
		...
		if (criteriaCount >= 4 && namedFiles.length >= 3) {
			sections.push(`\nWARNING: This task has ${criteriaCount} criteria and names ${namedFiles.length} files. You MUST edit at least ${namedFiles.length} files. Do NOT stop after editing fewer.`);
		} else if (criteriaCount >= 4) {
			const uniqueHitFiles = sorted.filter(([, kws]) => kws.size >= 2).length;
			if (uniqueHitFiles >= 3) {
				sections.push(`\nWARNING: This task has ${criteriaCount} criteria and ${uniqueHitFiles} files match multiple task keywords. Ensure you edit all required files before stopping.`);
			}
		}

		const topTargets = new Set<string>();
		for (const p of literalPaths.slice(0, 3)) topTargets.add(p);
		for (const [f] of sorted.slice(0, 3)) topTargets.add(f);
		const siblingInfo: string[] = [];
		const seenDirs = new Set<string>();
		for (const file of [...topTargets].slice(0, 3)) {
			const dir = file.replace(/\/[^/]+$/, "") || ".";
			if (seenDirs.has(dir)) continue;
			seenDirs.add(dir);
			try {
				const siblings = execSync(
					`ls -1 "${shellEscape(dir)}" 2>/dev/null | head -20`,
					{ cwd, timeout: 1500, encoding: "utf-8", maxBuffer: 512 * 1024 },
				).trim();
				if (siblings) {
					siblingInfo.push(`${dir}/: ${siblings.split("\n").join(", ")}`);
				}
			} catch {}
		}
		if (siblingInfo.length > 0) {
			sections.push("\nSIBLING FILES (check these directories for related files that may need changes):");
			for (const s of siblingInfo) sections.push(`  ${s}`);
		}
